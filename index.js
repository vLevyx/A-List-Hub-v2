...
// ✅ Initialize Supabase client before anything that uses it
...
async function getSessionWithRetry(retries = CONFIG.RETRY_ATTEMPTS, delay = CONFIG.RETRY_DELAY) {
    for (let attempt = 0; attempt < retries; attempt++) {
        const { data: { session }, error } = await client.auth.getSession();
        console.log(`[Session Retry] Attempt ${attempt + 1}:`, session ? '✅ Session Found' : '❌ Not Found');
...
// let statements would go back here
...
const AuthManager = {
    lastKnownSession: null, // Track the last known session to detect actual new logins
    ...
    setupListener() {
        ...
        client.auth.onAuthStateChange(async (event, session) => {
            ...
            if (session?.user) {
                ...
                // IMPROVED: Only handle sign-in for truly new logins
                // Check if this is a genuinely new login by comparing with last known session
                const isNewLogin = this.isNewLogin(event, session);

                if (isNewLogin) {
                    console.log("[🆕 AUTH] Detected new login, calling handleSignIn");
                    await this.handleSignIn(session);
                    this.lastKnownSession = session; // Update tracking
                }
            } else {
                userHasAccess = false;
                this.lastKnownSession = null; // Clear when logged out
            }

            // Update UI with proper timing
            const updateUI = () => {
    ...
    // NEW: Determine if this is actually a new login
    isNewLogin(event, session) {
        // Only consider SIGNED_IN events
        if (event !== "SIGNED_IN") {
            return false;
        }

        // If we have no previous session, this is definitely new
        if (!this.lastKnownSession) {
            return true;
        }

        // If the user ID changed, this is a new login
        if (this.lastKnownSession.user?.id !== session?.user?.id) {
            return true;
        }

        // Check if this login happened very recently (within last 5 seconds)
        // This helps distinguish actual OAuth redirects from session restorations
        const sessionCreatedAt = new Date(session.user.created_at);
        const timeSinceCreation = Date.now() - sessionCreatedAt.getTime();

        // If the session is very new OR we detect a significant time gap, likely new login
        const isVeryNewSession = timeSinceCreation < 30000; // 30 seconds
        const hasUrlParams = window.location.search.includes('access_token') ||
            window.location.search.includes('refresh_token') ||
            document.referrer.includes('discord.com');

        return isVeryNewSession && hasUrlParams;
    },

    // Handle sign-in RPC (unchanged)
    async handleSignIn(session) {
        const discordId = session.user.user_metadata.provider_id || session.user.user_metadata.sub;
        const username = session.user.user_metadata.full_name || 'Discord User';

        try {
            console.log("[🔍 DEBUG] Calling upsert_user_login for:", discordId);
            ...
            console.log("[🔍 DEBUG] upsert_user_login response:", data);

            // Log specific debug info
            if (data?.debug) {
                console.log("[🔍 DEBUG] Time since last login:", data.debug.time_since_last_login, "seconds");
                console.log("[🔍 DEBUG] Threshold:", data.debug.threshold_seconds, "seconds");
                console.log("[🔍 DEBUG] Should increment:", data.debug.should_increment);
                console.log("[🔍 DEBUG] Current login count:", data.debug.current_login_count);
            }
    ...
    // Enhanced logout function (unchanged)
    async logout() {
        ...
        currentSession = null;
        userHasAccess = false;
        this.lastKnownSession = null; // Clear tracking
